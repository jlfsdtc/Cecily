...
impl PostgresMetadataStore {
    /// Create a new PostgreSQL metadata store
    pub async fn new(database_url: &str) -> Result<Self> {
        let pool = PgPoolOptions::new()
            .max_connections(10)
            .connect(database_url)
            .await
            .map_err(|e| KylinError::Metadata(format!("Failed to connect to PostgreSQL: {}", e)))?;
...
impl MetadataStore for PostgresMetadataStore {
    ...
    async fn load_model(&self, project: &str, uuid: &str) -> Result<DataModel> {
        let row = sqlx::query(
            "SELECT definition FROM kylin_model WHERE project = $1 AND uuid = $2"
        )
        .bind(project)
        .bind(uuid)
        ...
        match row {
            Some(row) => {
                ...
                let model: DataModel = serde_json::from_value(definition)?;
                Ok(model)
            }
    ...
    async fn save_model(&self, project: &str, model: &DataModel) -> Result<()> {
        let definition = serde_json::to_value(model)?;
    ...
    async fn list_models(&self, project: &str) -> Result<Vec<DataModel>> {
        let rows = sqlx::query(
            "SELECT definition FROM kylin_model WHERE project = $1 ORDER BY name"
        )
        .bind(project)
        .fetch_all(&self.pool)
        .await
    ...
    async fn delete_model(&self, project: &str, uuid: &str) -> Result<()> {
        let result = sqlx::query(
            "DELETE FROM kylin_model WHERE project = $1 AND uuid = $2"
        )
        .bind(project)
        .bind(uuid)
        .execute(&self.pool)
    ...
    async fn load_dataflow(&self, uuid: &str) -> Result<Dataflow> {
        let row = sqlx::query(
            "SELECT definition FROM kylin_dataflow WHERE uuid = $1"
        )
        .bind(uuid)
        .fetch_optional(&self.pool)
        .await
        ...
        match row {
            Some(row) => {
                let definition: serde_json::Value = row.get("definition");
                let dataflow: Dataflow = serde_json::from_value(definition)?;
                Ok(dataflow)
            }
            None => Err(KylinError::NotFound(format!(
    ...
    async fn save_dataflow(&self, dataflow: &Dataflow) -> Result<()> {
        let definition = serde_json::to_value(dataflow)?;
        let now = chrono::Utc::now().timestamp_millis();
        ...
        sqlx::query(
            ...
            ON CONFLICT(uuid) DO UPDATE SET
                ...
                definition = EXCLUDED.definition,
                last_modified = EXCLUDED.last_modified,
                version = kylin_dataflow.version + 1
            "#
        )
    ...
    async fn list_dataflows(&self, project: &str) -> Result<Vec<Dataflow>> {
        let rows = sqlx::query(
            "SELECT definition FROM kylin_dataflow WHERE project = $1 ORDER BY model_name"
        )
        .bind(project)
        .fetch_all(&self.pool)
        .await
    ...
    async fn delete_dataflow(&self, uuid: &str) -> Result<()> {
        let result = sqlx::query(
            "DELETE FROM kylin_dataflow WHERE uuid = $1"
        )
        .bind(uuid)
        .execute(&self.pool)
        .await
    ...
    async fn load_segment(&self, uuid: &str) -> Result<Segment> {
        let row = sqlx::query(
            "SELECT definition FROM kylin_segment WHERE uuid = $1"
        )
        .bind(uuid)
        .fetch_optional(&self.pool)
        .await
        ...
        match row {
            Some(row) => {
                let definition: serde_json::Value = row.get("definition");
                let segment: Segment = serde_json::from_value(definition)?;
                Ok(segment)
            }
            None => Err(KylinError::NotFound(format!(
    ...
    async fn save_segment(&self, segment: &Segment) -> Result<()> {
        let definition = serde_json::to_value(segment)?;
        let now = chrono::Utc::now().timestamp_millis();
        ...
        sqlx::query(
            ...
            ON CONFLICT(uuid) DO UPDATE SET
                ...
                definition = EXCLUDED.definition,
                last_modified = EXCLUDED.last_modified,
                version = kylin_segment.version + 1
            "#
        )
    ...
    async fn list_segments(&self, dataflow_uuid: &str) -> Result<Vec<Segment>> {
        let rows = sqlx::query(
            "SELECT definition FROM kylin_segment WHERE dataflow_uuid = $1 ORDER BY time_range_start"
        )
        .bind(dataflow_uuid)
        .fetch_all(&self.pool)
        .await
    ...
    async fn delete_segment(&self, uuid: &str) -> Result<()> {
        let result = sqlx::query(
            "DELETE FROM kylin_segment WHERE uuid = $1"
        )
        .bind(uuid)
        .execute(&self.pool)
        .await
    ...
    async fn load_table(&self, project: &str, full_name: &str) -> Result<TableDesc> {
        let row = sqlx::query(
            "SELECT definition FROM kylin_table_desc WHERE project = $1 AND full_name = $2"
        )
        .bind(project)
        .bind(full_name)
        .fetch_optional(&self.pool)
        ...
        match row {
            Some(row) => {
                let definition: serde_json::Value = row.get("definition");
                let table: TableDesc = serde_json::from_value(definition)?;
                Ok(table)
            }
            None => Err(KylinError::NotFound(format!(
    ...
    async fn save_table(&self, project: &str, table: &TableDesc) -> Result<()> {
        let definition = serde_json::to_value(table)?;
        let now = chrono::Utc::now().timestamp_millis();
    ...
    async fn list_tables(&self, project: &str) -> Result<Vec<TableDesc>> {
        let rows = sqlx::query(
            "SELECT definition FROM kylin_table_desc WHERE project = $1 ORDER BY full_name"
        )
        .bind(project)
        .fetch_all(&self.pool)
        .await
    ...
    async fn load_project(&self, name: &str) -> Result<Project> {
        let row = sqlx::query(
            "SELECT definition FROM kylin_project WHERE name = $1"
        )
        .bind(name)
        .fetch_optional(&self.pool)
        .await
        ...
        match row {
            Some(row) => {
                let definition: serde_json::Value = row.get("definition");
                let project: Project = serde_json::from_value(definition)?;
                Ok(project)
            }
            None => Err(KylinError::NotFound(format!(
    ...
    async fn save_project(&self, project: &Project) -> Result<()> {
        let definition = serde_json::to_value(project)?;
        let now = chrono::Utc::now().timestamp_millis();
        ...
        sqlx::query(
            ...
            ON CONFLICT(uuid) DO UPDATE SET
                ...
                definition = EXCLUDED.definition,
                last_modified = EXCLUDED.last_modified,
                version = kylin_project.version + 1
            "#
        )
    ...
    async fn delete_project(&self, name: &str) -> Result<()> {
        let result = sqlx::query(
            "DELETE FROM kylin_project WHERE name = $1"
        )
        .bind(name)
        .execute(&self.pool)
        .await
